...
% This demo is used to reproduce the sound field results presented in Fig.9 of the paper 
...
% Environment setup
[grid, env, casename] = read_env('input4.env'); % Load env file

% Source beam pattern (double pistons)
phi_s    = 15*pi/180; 
ra       = 10;
sbp.dir  = @(varphi, k) ...
    2*besselj(1, k*ra*sin(varphi-phi_s))./(k*ra*sin(varphi-phi_s)) + ...
    2*besselj(1, k*ra*sin(varphi+phi_s))./(k*ra*sin(varphi+phi_s));
